...
export function registerPrivateChatHandlers(io: Server, socket: Socket) {
  ...
  socket.on('getUnreadCounts', async ({ roomId, userId }) => {
  ...
  socket.on('markAsRead', async ({ roomId, userId, otherUserId }) => {
  ...
  socket.on('fetchPrivateChat', async ({ roomId, userA, userB }) => {
  ...
  socket.on('sendPrivateMessage', async (data) => {
    ...
    try {
      ...
      try {
        await PrivateChat.findOne({}).limit(1);
      ...
      let chat: IPrivateChatDocument | null = await PrivateChat.findOne({
        roomId,
        userA,
        userB
      });
